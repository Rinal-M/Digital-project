...
>>>>>>> ab7a004ae8a8ac7ebaa3dc9d268a828a12fc30b6
...
vector<Reps> compare(vector< vector<Reps>> groups)
{
    ...
    for (int c=0; c<16; c++) // this 16 will be changed later after the code is completed
    {
        ...
        for (int i=0; i< groups[c].size(); i++)
        {
            for (int j=0; j<groups[c+1].size(); j++)
            {
                if (Bitdiff( x[i]->binary,y[j]->binary)) //assuming that we have a //function  called bitdiff
                {
                    flag =true;
                    ...
                    y[j].primecheck=1;
                    Reps temp;
                    temp.binary=D_replace(x[i],y[j]);
                    if (x[i].comb.size()!=0)
                    {
                        temp.comb=x[i].comb;   // just copying content of vector x[i] into temp
                        temp.comb.insert(temp.comb.end(), y[j].comb.begin(), y[j].comb.end());
                    }
                    else
                        
                        temp.comb.push_back(x[i].minterm);
                    temp.comb.push_back(y[j].minterm);
